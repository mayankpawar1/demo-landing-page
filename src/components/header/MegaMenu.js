...
const MegaMenu = () => {
  return (
    <div
      className="z-50 w-full bg-white px-48"
      role="region"
      aria-expanded="true"
      aria-hidden="false"
    >
  ...
  return (
    <ul className="space-y-3">
      {items.map(([title, path, subheading = false, isExternal = false,]) => (
        <li key={title}>
          <a
            className={`${subheading && "pl-8"} "text-[#003145] hover:underline items-center transition duration-200 block text-2lg"`}
            href={isExternal ? path : `${baseUrl}${path}`}
            target={isExternal ? "_blank" : ""}
            rel={isExternal ? "noopener noreferrer" : ""}
            // className="text-gray-700 hover:text-blue-800 transition duration-200 block"
          >
            {title}
          </a>
        </li>
